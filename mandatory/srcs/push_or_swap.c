...
/*   Updated: 2022/02/02 17:52:00 by bdetune          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static void	pushinorder(t_info *info, t_moves *possibility)
{
	int		nb;
	int		dist_b;
	t_list	*it;
	...
	{
		bringtofront(info, possibility, 'a');
		possibility->pb += 1;
		return ;
	}
	dist_b = 0;
...
void	push_or_swap(t_info *info, t_moves *mv)
{
	...
	{
		bringtofront(info, &pos_sa_nxt, 'a');
		pos_sa_nxt.sa += 1;
	}
